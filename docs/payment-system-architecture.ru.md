...
6. Обычная операция без разрешения администратора не может списать больше доступного остатка.
7. Отрицательный денежный баланс допустим как последствие внешнего refund, chargeback или явно разрешённого администратором списания консультации.
...
14. Дату и время оплаченной встречи можно менять без нового списания, если длительность сохраняется.
    Изменение длительности требует отмены с автоматической компенсацией и создания новой встречи.
15. Запрос переноса хранит явную ссылку на исходную встречу. До подтверждения исходный слот остаётся занятым;
    после подтверждения он освобождается в той же транзакции, где компенсируется прежняя оплата и фиксируется новая.
...
Администратор задаёт стоимость 60 минут консультации в EUR на странице «Платежи». Настройка хранится в `ConsultationRate` с полями `amount`, `currency = EUR`, `updatedById` и временными метками.

При создании встречи из денежного баланса стоимость рассчитывается пропорционально её фактической длительности и округляется до центов. В allocation сохраняется снимок цены и длительности. Длительность также определяет количество списанных пакетных минут. Пересчёт старых встреч после изменения цены запрещён.
...
Незавершённая локальная provider-запись может быть удалена только отдельной административной
операцией после точного `not found` от исходного коннектора. Проверка 404 является частью
provider adapter: транспортные, auth и 5xx-ошибки пробрасываются и блокируют удаление. Записи с
capture, fulfilment, ledger, webhook events, disputes или refunds физически не удаляются.
...
- Незавершённый ошибочный payment удалять только после `not found` у исходного провайдера,
  повторной проверки отсутствия локальных финансовых связей и записи в технический audit-log.
- Provider refund разрешать только для завершённого `TOPUP`; внутренний баланс уменьшать
  подтверждённой провайдером компенсирующей проводкой.

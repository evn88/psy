...
Единственный источник разметки production-писем — универсальная система в `src/modules/email-templates/`.
`src/emails` содержит только тесты и вспомогательный код для проверки писем; новые React Email-компоненты, самостоятельные
HTML-строки и файлы шаблонов для отдельных сценариев создавать нельзя.

Для нового сценария:

1. Зарегистрируй сценарий и его токены в `email-template-registry.ts`.
2. Добавь дефолтные `subject`, `html` и `css` в реестр.
3. Передавай значения токенов из отправителя через `renderStoredEmailTemplate()`.
4. Добавь или обнови тесты рендера в `src/emails/__tests__/`.

Отправитель отвечает за данные и адресатов, но не за разметку. Общие шапка, футер, санитизация пользовательского HTML и
инлайнинг CSS принадлежат `email-template-renderer.server.ts` и не должны дублироваться в сценариях.

Токен обязан быть зарегистрирован с типом `text`, `url` или `html`. Текстовые значения экранируются, URL принимают
только безопасные HTTP(S)-адреса, а `html` разрешён исключительно для доверенного серверного содержимого после
санитизации.

Пользовательские стили хранятся в поле `css` и действуют внутри `.email-content`. Для элементов с внутренними
отступами нельзя задавать `width: 100%`, если это расширяет блок за пределы контейнера: используй безопасную для
email-вёрстки структуру или табличную раскладку. Проверяй переполнение регрессионным тестом.

Для ручной проверки используй встроенную тестовую отправку или `scripts/manual/send-email-previews.ts`; получателей
нельзя хардкодить в исходном коде.

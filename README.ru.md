...
> В проекте используется только npm. Используйте `package-lock.json` как единственный lock-файл: `npm ci` для
> воспроизводимой установки и `npm install` для изменения зависимостей. Установка через pnpm, yarn или bun блокируется.
> Подробнее: [политика пакетного менеджера](docs/package-manager.ru.md).

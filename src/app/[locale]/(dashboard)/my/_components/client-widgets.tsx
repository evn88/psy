...
export const NotesWidget: WidgetComponentType = ({ isEditing }) => {
  const t = useTranslations('My');
  ...
  useEffect(() => {
    ...
    setIsMounted(true);
    const saved = localStorage.getItem('my_dashboard_notes');
    if (saved) setNote(saved);
  }, []);

  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    setNote(e.target.value);
    localStorage.setItem('my_dashboard_notes', e.target.value);
  };

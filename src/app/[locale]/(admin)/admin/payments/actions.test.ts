...
describe('административные действия с платежами', () => {
  ...
  it('возвращает завершённое пополнение через исходного провайдера', async () => {
    ...
    const result = await refundPaymentAction('payment-1', '50.00', idempotencyKey);
  ...
  it('возвращает указанную часть завершённого пополнения через исходного провайдера', async () => {
    // Arrange
    mocks.findUnique.mockResolvedValue(completedTopup);
    mocks.refundPayment.mockResolvedValue(completedTopup);

    // Act
    const result = await refundPaymentAction('payment-1', '18.50', idempotencyKey);
  ...
  it('не позволяет вернуть сумму больше доступного остатка', async () => {
    // Arrange
    ...
    // Act
    const result = await refundPaymentAction('payment-1', '20.00', idempotencyKey);

    // Assert
    expect(result).toEqual({
      success: false,
      message: 'Сумма возврата превышает доступный остаток 10.00 EUR'
    });

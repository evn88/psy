  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (state.axisXOffset + viewportWidth > state.lastPipeX + 400) {
        ...
        if (refs.bottomSeriesRef.current) refs.bottomSeriesRef.current.data.setAll(cleanPipes);
        if (refs.topSeriesRef.current) refs.topSeriesRef.current.data.setAll(cleanPipes);
      }
